...
function AddressController(opts) {
  ...
  this.pushDeviceRepository = opts.pushDeviceRepository;
}
...
AddressController.prototype.transformUtxo = function(utxoArg) {
  ...
  if (utxoArg.height && utxoArg.height > 0) {
    ...
    utxo.confirmations = this.node.services.silubiumd.height - utxoArg.height + 1;
  } else {
...
AddressController.prototype.savedevice = function(req, res, next) {
    var self = this;

    self.common.log.info('请求参数为 ******',req.body);
    var device = {platform: req.body.platform,
        tag:req.body.tag,
    ...
    this.pushDeviceRepository.createOrUpdateDevice(device,function(err, data) {
        if (err) {
            return self.common.handleErrors(err, res);
        }
        res.jsonp('true');
