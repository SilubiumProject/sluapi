...
BlockController.prototype.getBlockByHash = function (hash, next) {
    ...
    if (blockCached) {
        ...
        blockCached.confirmations = self.node.services.silubiumd.height - blockCached.height + 1;
        ...
        return async.waterfall([function (callback) {
            ...
            return self.node.services.silubiumd.getBlockHeader(hash, function (err, info) {
...
BlockController.prototype.blockIndex = function (req, res) {
    ...
    this.node.services.silubiumd.getBlockHeader(parseInt(height), function (err, info) {
        if (err) {
            return self.common.handleErrors(err, res);
        }
        res.jsonp({
...
BlockController.prototype._getBlockSummary = function (hash, moreTimestamp, next) {
    ...
    if (summaryCache) {
        ...
        return async.waterfall([function (callback) {
            return self.node.services.silubiumd.client.getBlock(hash, function (err, response) {
            ...
            var confirmations = self.node.services.silubiumd.height - block.height + 1;
...
BlockController.prototype.list = function (req, res) {
    ...
    return self.node.services.silubiumd.getBlockHashesByTimestamp(lte, gte, function (err, hashes) {
