...
StatisticsController.prototype.listBalanceByAddresses = function(req, res) {

    var self = this,
    balances = [];
    return this.addressBalanceService.listBalanceByAddresses(req.addrs,function (err, items) {

        if (err) {
            return self.common.handleErrors(err, res);
        }
        items.forEach(function (item) {
            balances.push({address:item.address,balance:item.balance});
        })
        return res.jsonp(balances);
