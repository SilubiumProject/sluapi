...
module.exports = {
    ...
    isSrc20Contract: function (scriptHex) {
        var hashes = [
    ...
    getEthAddressFromBitAddress: function (bitAddress) {
        ...
        if(bitAddress.startsWith('SL')){
            bitAddress = bitAddress.substring(2);
        }
        try {
            var hexBitAddress = bitcore.encoding.Base58.decode(bitAddress).toString('hex');
            return hexBitAddress.slice(2, -8);

        } catch (e) {
            console.log('hexBitAddress 3:'+hexBitAddress)
            return null;
        }
    ...
    getBitAddressFromContractAddress: function (contractAddress, networkId) {
        try {
            ...
            return 'SL'+bitcore.encoding.Base58.encode(new bitcore.deps.Buffer(hexBitAddress, 'hex'));
        } catch (e) {
