 ...
 * @return {*}
 */
AddressBalanceService.prototype.listBalanceByAddresses = function (addresses, next) {

    var self = this;

    self.addressBalanceRepository.listBalanceByAddresses(addresses,function (err,result) {
        if(err){
            self.common.log.error('listBalanceByAddresses error');
        }else{
...
AddressBalanceService.prototype._rapidProtectedUpdateTip = function (height) {
    ...
    return this._processLastBlocks(height, function (err) {
        ...
        self.client.push().setPlatform('ios', 'android')
            .setAudience(jPush.JPushAsync.ALL)
