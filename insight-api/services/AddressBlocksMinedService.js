...
AddressBlocksMinedService.prototype.processBlock = function (blockHeight, next) {
    ...
    return async.waterfall([function (callback) {
        return self.node.services.silubiumd.getJsonBlock(blockHeight, function (err, response) {
        ...
        switch (block.flags) {
            case silubiumcore.Block.PROOF_OF_STAKE:
                txHash = block.tx[1];
                break;

            case silubiumcore.Block.PROOF_OF_WORK:
                txHash = block.tx[0];
                break;
        }
